...
export class AddWorkerSlideComponent extends React.Component<Props> {
  ...
  getError = (pair: [number, string][]) => {
    const key = pair.reduce(
      (errorKey: string, [idx, key]) => {
        errorKey += `[${idx}].${key}`;
        return errorKey;
      },
      '',
  ...
  addNewWorker = () => {
    this.workers = [...this.workers, new AssignWorker({ startDate: this.props.selectedDate })];
    this.forceUpdate();
  };
          ...
          <div className="slide-body">
                    ...
                    <>
                      <div className="">
                        ...
                        <SubcontractorAsyncSearch
                          defaultInputValue={item.subcontractor && item.subcontractor.name}
                          onSelect={(item) => this.onSubcontractorSelect(index, item)}
                      ...
                      <CheckboxComponent className="" checked={this.onlyAvailable} onChange={(checked) => this.searchWorkers(checked)} id="scheduleCheck"
                      ...
                      <div className="ce-assign-worker-record work-time">
                        <div className="work-time-start-date">
                          <DateComponent
                            hasTitle="Start Date"
                            showTimeSelect
                            minDate={this.props.job.requestTime ?
                            ...
                            <div key={`location${index}${item.location}`}>
                              <div className="work-time-start-time">
                                <Select
                                  ...
                                  defaultInputValue={item.location && item.location.address}
                                ...
                                />
                                <p>{this.getError([[index, 'location.address']])}</p>
                              </div>
                            </div> : null
                        }
                      </div>
            ...
            <div className="worker-action">
              <div className="ce-assign-worker-record">
                <a className="btn ce-btn-confirm cursor-pointer" onClick={this.addNewWorker}>
